    ...
    P: OutputPin + Send + 'static,
{
    ...
    pub fn init(driver: Arc<Mutex<Self>>, sys_loop: EspEventLoop<System>) {
        ...
        driver.lock().unwrap().timer = timer;
    }
    ...
    pub fn start_auto_capture(
        ...
        if let Ok(mut guard) = driver.lock() {
            if let Some(ref timer) = guard.timer {
                guard.sys_loop.post::<SystemEvent>(&SystemEvent::AutoCaptureStarted, delay::BLOCK).unwrap();
    ...
    pub fn stop_auto_capture(
        ...
        if let Ok(mut guard) = driver.lock() {
            if let Some(ref timer) = guard.timer {
                guard.sys_loop.post::<SystemEvent>(&SystemEvent::AutoCaptureStopped, delay::BLOCK).unwrap();
                if !timer.is_scheduled().unwrap() {
                    return;
                }
                timer.cancel().unwrap();
